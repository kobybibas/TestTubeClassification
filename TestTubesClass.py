...
class TestTubes:
    ...
    def mainFunc(self, img):
        ...
        self.red_lower_bound     = np.array([170, 50, 15])
        self.red_upper_bound     = np.array([10, 200, 220])
        self.blue_lower_bound    = np.array([90, 20, 5])
        self.blue_upper_bound    = np.array([115, 150, 100])
        self.brown_lower_bound   = np.array([3, 100, 80])
        self.brown_upper_bound   = np.array([15, 215, 245])
    ...
    def buildTestTubesDB(self, contour_db):
        ...
        for k in range(0, contour_db.shape[0]):
            ...
            test_tubes_db.loc[k, 'litmus_y_min'] = test_tubes_db.loc[k, 'cork_y_min'] + test_tubes_db.loc[k, 'cork_height'] + litmus_start_var * test_tubes_db.loc[k, 'cork_height']
            test_tubes_db.loc[k, 'litmus_width'] = contour_db.loc[k, 'width'] * litmus_width_ratio   # Same width as th cork
            test_tubes_db.loc[k, 'litmus_height'] = litmus_height_var * test_tubes_db.loc[k, 'cork_height']

            test_tubes_db.loc[k, 'litmus_x_min'] = test_tubes_db.loc[k, 'litmus_center_x'] - test_tubes_db.loc[k, 'litmus_width']/2  # same x_min as th cork
